...
const ExpensePieChart = ({ data, monthlyData, selectedMonth, onMonthChange }) => {
  ...
  const filteredData = useMemo(() => {
    ...
    const categories = Object.entries(monthData.byCategory)
      .filter(([_, values]) => values.expenses > 0)
      .map(([category, values]) => ({
        category,
        total: values.expenses,
        count: values.count
      }))
      ...
      <Paper sx={{ p: 4, textAlign: 'center', height: '100%' }}>
        <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
          <Box display="flex" alignItems="center">
            <PieChartIcon sx={{ fontSize: 28, mr: 2, color: '#ff6b6b' }} />
  ...
  const chartData = formatPieChartData(filteredData, 'expenses');
  const chartOptions = getPieChartOptions();

  const totalExpenses = filteredData.reduce((sum, c) => sum + c.total, 0);
