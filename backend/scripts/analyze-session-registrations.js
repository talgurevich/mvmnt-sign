...
/**
 * Analyze Session Registrations from Shift Summary Report
 *
 * Parses CSV data and creates a pie chart showing session registrations
 */
...
function analyzeSessionRegistrations() {
  console.log('📊 Analyzing Session Registrations...\n');
  ...
  // Count registrations by session (with consolidation)
  const sessionCounts = {};
  ...
  // Display summary
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  console.log('Session Registration Summary (Consolidated):');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');

  sortedSessions.forEach(session => {
    const percentage = ((session.count / data.length) * 100).toFixed(1);
    console.log(`${session.name}: ${session.count} registrations (${percentage}%)`);
  });

  console.log('\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');

  return { totalRegistrations: data.length, sessionCounts: sortedSessions };
}
...
<head>
    ...
    <title>Session Registrations - Last 30 Days</title>
...
<body>
    <div class="container">
        <h1>📊 התפלגות הרשמות לשיעורים</h1>
        <p class="subtitle">Consolidated Session Registrations - Last 30 Days</p>

        <div class="stats-grid">
            <div class="stat-card">
                <div class="stat-value">${stats.totalRegistrations}</div>
                <div class="stat-label">סך הכל הרשמות</div>
            </div>
            <div class="stat-card">
                <div class="stat-value">${stats.sessionCounts.length}</div>
                <div class="stat-label">סוגי שיעורים</div>
            </div>
            <div class="stat-card">
                <div class="stat-value">${stats.sessionCounts[0]?.name || 'N/A'}</div>
                <div class="stat-label">השיעור הפופולרי ביותר</div>
            </div>
        </div>
        ...
        <div class="footer">
            Generated on ${new Date().toLocaleString('he-IL')} | Data from Arbox Shift Summary Report
        </div>
    </div>
  ...
  // Save HTML file
  const outputPath = path.join(__dirname, '..', 'session-registrations-chart.html');
...
if (require.main === module) {
  try {
    const stats = analyzeSessionRegistrations();
    const chartPath = generatePieChart(stats);
...
module.exports = { analyzeSessionRegistrations, generatePieChart };
